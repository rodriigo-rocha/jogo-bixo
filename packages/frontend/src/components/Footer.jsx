    ...
    const fetchWeather = async () => {
      try {
        const response = await fetch(
          "https://api.weatherapi.com/v1/current.json?key=YOUR_API_KEY&q=Sao%20Paulo&aqi=no",
    ...
    <footer className="bg-gray-300 h-[7dvh] flex justify-between w-dvw px-5 absolute bottom-0 align-middle select-none cursor-pointer">
      ...
      <div className="flex flex-col items-end text-right">
        ...
        <div className="flex items-center gap-1 text-sm text-gray-500">
          {weather.icon && (
            <img src={weather.icon} alt="clima" className="w-4 h-4" />
          )}
          <span>
      ...
      {showMenu && (
        <div className="absolute bottom-full left-5 bg-gray-200 border border-gray-400 shadow-lg z-50">
          ...
          <div
            ...
            onClick={handleKoala}
          >
            E coalas? Você gosta de coalas? 🐨
          </div>
          <div
            className="p-2 cursor-pointer hover:bg-gray-300"
            onClick={handleHarleyDavidson}
          >
            Conheça a Harley Davidson 🏍
          </div>
          <div
            className="p-2 cursor-pointer hover:bg-gray-300"
            onClick={handleLionelMessi}
          >
            Vinhos Lionel Messi 🍷
          </div>
        </div>
